...
#define TAG "jp.co.aircast.module"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Modulesample_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Modulesample.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::ModulesampleBindings::lookupGeneratedInit(
...
static void Modulesample_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Modulesample_getBinding)->GetFunction());
}

static void Modulesample_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::ModulesampleBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry ModulesampleBinding = {
	"jp.co.aircast.module",
	Modulesample_init,
	Modulesample_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_jp_co_aircast_module_ModulesampleBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("jp.co.aircast.module", &ModulesampleBinding);
	titanium::KrollBindings::addExternalLookup(&(::ModulesampleBindings::lookupGeneratedInit));
}
