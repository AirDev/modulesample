 ...
 *
 */
public final class LinphoneService extends Service implements LinphoneServiceListener, LinphoneOnTextReceivedListener {
...
//		mNotif = new Notification(R.get("drawable", "status_level"), "", System.currentTimeMillis());
...
//		notifIntent.setClassName(ALModuleProxy.PKG_NAME, ALModuleProxy.PKG_NAME + "." + ALModuleProxy.ACT_NAME);
...
//			return;
//		case INCALL:
//			mIncallNotif.icon = R.get("drawable", "conf_unhook");
//			notificationTextId = R.get("string", "incall_notif_active");
//			break;
//		case PAUSE:
//			mIncallNotif.icon = R.get("drawable", "conf_status_paused");
//			notificationTextId = R.get("string", "incall_notif_paused");
//			break;
//		case VIDEO:
//			mIncallNotif.icon = R.get("drawable", "conf_video");
//			notificationTextId = R.get("string", "incall_notif_video");
//			break;	
//		default:
...
//		});
	...
	public void onGlobalStateChanged(final GlobalState state, final String message) {
		...
		//	sendNotification(IC_LEVEL_OFFLINE, R.get("string", "notification_started"));
...
//			sendNotification(IC_LEVEL_ORANGE, R.get("string", "notification_registered"));
//		}

//		if (state == RegistrationState.RegistrationFailed || state == RegistrationState.RegistrationCleared) {
//			sendNotification(IC_LEVEL_OFFLINE, R.get("string", "notification_register_failure"));
//		}

		if (state == RegistrationState.RegistrationOk || state == RegistrationState.RegistrationFailed) {
			mHandler.post(new Runnable() {
				public void run() {
					ALModuleProxy.onRegistrationStateChanged(state, message);
...
//		notifIntent.setClassName(ALModuleProxy.PKG_NAME, ALModuleProxy.PKG_NAME + "." + ALModuleProxy.ACT_NAME);
//		notifIntent.setClassName("jp.co.aircast.saito.alDev", "jp.co.aircast.saito.alDev.Al_devActivity");
...
//		});
	...
	public void changeRingtone(String ringtone) {
		...
		editor.putString(getString(R.get("string", "pref_audio_ringtone")), ringtone);
		editor.commit();
	}

	public void onRingerPlayerCreated(MediaPlayer mRingerPlayer) {
		String uriString = PreferenceManager.getDefaultSharedPreferences(this).getString(getString(R.get("string", "pref_audio_ringtone")), 
...
//		Object[] obj = {from.toString(), message};
	...
	Handler myHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			Log.d("myHandler:type = " + type);
			
			if(type.equals("invite") == true)
			{
				String fromUrl = bundle.getString("fromUrl");
				String fromName = bundle.getString("fromName");
				String callType = bundle.getString("callType");
				
				userNameText = fromName;
				callTypeText = callType;
				
				Log.d("myHandler:fromUrl = " + fromUrl);
				Log.d("myHandler:fromName = " + fromName);
				
				try {
					
					DefaultHttpClient httpClient = new DefaultHttpClient();
					HttpGet hg = new HttpGet(fromUrl);
	...
	public void postInfo(String url, String name, String callType)
	{
		userImage = null;
		userNameText  = null;
		callTypeText  = null;
