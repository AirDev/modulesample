...
public abstract class AbstractContactPickerActivityNew extends Activity implements FilterQueryProvider {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		useNativePicker = getResources().getBoolean(R.get("bool", "use_android_contact_picker"));

		if (!useNativePicker) {
			setContentView(R.get("layout", "contact_picker"));
			createCustomPicker();
		}
	...
	protected void createCustomPicker() {
        mContactList = (ListView) findViewById(R.get("id", "contactList"));

        mcontactFilter = (EditText) findViewById(R.get("id", "contactFilter"));
...
//			((ContactPicked) getParent()).goToDialer();
		...
		default:
			...
			builder.setTitle(String.format(getString(R.get("string", "title_numbers_dialog")),contactName));
