...
public class LinphonePreferencesActivity extends PreferenceActivity implements EcCalibrationListener {
	...
	private void detectAudioCodec(int id, String mime, int rate, boolean hide) {
		...
		if (hide && !enable) {
			cb.setLayoutResource(R.get("layout", "hidden"));
		}
	}
	...
	private void createDynamicAccountsPreferences() {
		...
		nbAccounts = prefs.getInt(getString(R.get("string", "pref_extra_accounts")), 0);
...
//	        	addExtraAccountPreferencesButton(accounts, nbAccounts, true);
	...
	private void addExtraAccountPreferencesButton(PreferenceCategory parent, final int n, boolean isNewAccount) {
		...
		if (isNewAccount) {
			...
			editor.putInt(getString(R.get("string", "pref_extra_accounts")), n+1);
			editor.commit();
		}
		
		Preference me = new Preference(LinphonePreferencesActivity.this);
		String keyUsername = getString(R.get("string", "pref_username_key"));
		String keyDomain = getString(R.get("string", "pref_domain_key"));
		if (n > 0) {
			keyUsername += n + "";
			keyDomain += n + "";
		}
		if (prefs.getString(keyUsername, null) == null) {
			me.setTitle(getString(R.get("string", "pref_sipaccount")));
		} else {
...
//		parent.addPreference(me);
	...
	private void fillLinphoneAccount(int i, String username, String password, boolean createdByWizard) {
		...
		editor.putString(getString(R.get("string", "pref_username_key")) + i, username);
		editor.putString(getString(R.get("string", "pref_passwd_key")) + i, password);
		editor.putString(getString(R.get("string", "pref_domain_key")) + i, "sip.linphone.org");
		editor.putString(getString(R.get("string", "pref_proxy_key")) + i, "");
		editor.putBoolean(getString(R.get("string", "pref_wizard_key")) + i, createdByWizard);
		editor.putBoolean(getString(R.get("string", "pref_activated_key")) + i, false);
		editor.putBoolean(getString(R.get("string", "pref_enable_outbound_proxy_key")) + i, false);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == ADD_SIP_ACCOUNT) {
			...
			SharedPreferences prefs = getPreferenceManager().getSharedPreferences();
			int n = prefs.getInt(getString(R.get("string", "pref_extra_accounts")), 1);
			String keyUsername = getString(R.get("string", "pref_username_key")) + (n-1 == 0 ? "" : Integer.toString(n-1));
...
//			createDynamicAccountsPreferences();
	...
	protected Dialog onCreateDialog (int id) {
		if (id == WIZARD_ID) {
	    	...
	    	View v = inflater.inflate(R.get("layout", "wizard"), null);
	    	builder.setView(v);
	    	
	    	final EditText username = (EditText) v.findViewById(R.get("id", "wizardUsername"));
	    	ImageView usernameOkIV = (ImageView) v.findViewById(R.get("id", "wizardUsernameOk"));
	    	addXMLRPCUsernameHandler(username, usernameOkIV);

	    	final EditText password = (EditText) v.findViewById(R.get("id", "wizardPassword"));
	    	EditText passwordConfirm = (EditText) v.findViewById(R.get("id", "wizardPasswordConfirm"));
	    	ImageView passwordOkIV = (ImageView) v.findViewById(R.get("id", "wizardPasswordOk"));
	    	addXMLRPCPasswordHandler(password, passwordConfirm, passwordOkIV);

	    	final EditText email = (EditText) v.findViewById(R.get("id", "wizardEmail"));
	    	ImageView emailOkIV = (ImageView) v.findViewById(R.get("id", "wizardEmailOk"));
	    	addXMLRPCEmailHandler(email, emailOkIV);

	    	errorMessage = (TextView) v.findViewById(R.get("id", "wizardErrorMessage"));
	    	
	    	Button cancel = (Button) v.findViewById(R.get("id", "wizardCancel"));
	    	...
	    	createAccount = (Button) v.findViewById(R.get("id", "wizardCreateAccount"));
	    	...
	    	builder.setTitle(getString(R.get("string", "wizard_title")));
	    	wizardDialog = builder.create();
	    	return wizardDialog;
		}
		else if (id == CONFIRM_ID) {
			AlertDialog.Builder builder = new AlertDialog.Builder(LinphonePreferencesActivity.this);
			builder.setTitle(R.get("string", "wizard_confirmation"));
			
			final LayoutInflater inflater = LayoutInflater.from(LinphonePreferencesActivity.this);
	    	View v = inflater.inflate(R.get("layout", "wizard_confirm"), null);
	    	builder.setView(v);
	    	
	    	Button check = (Button) v.findViewById(R.get("id", "wizardCheckAccount"));
	    	check.setOnClickListener(new OnClickListener() {
				public void onClick(View v) {
					...
					if (isAccountVerified(username)) {
						SharedPreferences prefs = getPreferenceManager().getSharedPreferences();
						SharedPreferences.Editor editor = prefs.edit();
						editor.putBoolean(getString(R.get("string", "pref_activated_key")) + (nbAccounts - 1), true);
						editor.commit();
					} else {
	...
	private void isUsernameRegistred(String username, final ImageView icon) {
		final Runnable runNotReachable = new Runnable() {
			public void run() {
				errorMessage.setText(R.get("string", "wizard_server_unavailable"));
				usernameOk = false;
				icon.setImageResource(R.get("drawable", "notok"));
		...
		try {
			...
			XMLRPCCallback listener = new XMLRPCCallback() {
				Runnable runNotOk = new Runnable() {
    				public void run() {
    					errorMessage.setText(R.get("string", "wizard_username_unavailable"));
    					usernameOk = false;
						icon.setImageResource(R.get("drawable", "notok"));
						createAccount.setEnabled(usernameOk && passwordOk && emailOk);
					}
	    		...
	    		Runnable runOk = new Runnable() {
    				public void run() {
    					...
    					icon.setImageResource(R.get("drawable", "ok"));
						usernameOk = true;
						createAccount.setEnabled(usernameOk && passwordOk && emailOk);
					}
	    		};
	...
	private void createAccount(final String username, final String password, String email, boolean suscribe) {
		...
		try {
			...
			XMLRPCCallback listener = new XMLRPCCallback() {
				Runnable runNotOk = new Runnable() {
    				public void run() {
    					errorMessage.setText(R.get("string", "wizard_failed"));
					}
	    		};
...
//			    		createDynamicAccountsPreferences();
		...
		field.addTextChangedListener(new TextWatcher() {
					...
					int arg3) 
			{
				...
				if (isUsernameCorrect(field.getText().toString()))
				{
					...
					errorMessage.setText(R.get("string", "wizard_username_incorrect"));
					icon.setImageResource(R.get("drawable", "notok"));
				}
			}
		});
		...
		field.addTextChangedListener(new TextWatcher() {
					...
					int arg3) 
			{
				...
				if (isEmailCorrect(field.getText().toString())) {
					...
					errorMessage.setText(R.get("string", "wizard_email_incorrect"));
					icon.setImageResource(R.get("drawable", "notok"));
				}
				createAccount.setEnabled(usernameOk && passwordOk && emailOk);
			}
					...
					int arg3) 
			{
				...
				if (isPasswordCorrect(field1.getText().toString()) && field1.getText().toString().equals(field2.getText().toString())) {
					...
					if (isPasswordCorrect(field1.getText().toString())) {
						errorMessage.setText(R.get("string", "wizard_passwords_unmatched"));
					}
					else {
						errorMessage.setText(R.get("string", "wizard_password_incorrect"));
					}
					icon.setImageResource(R.get("drawable", "notok"));
				}
				createAccount.setEnabled(usernameOk && passwordOk && emailOk);
	...
	private void verifiyAccountsActivated() {
		...
		for (int i = 0; i < nbAccounts; i++) {
			...
			boolean createdByWizard = prefs.getBoolean(getString(R.get("string", "pref_wizard_key")) + key, false);
			boolean activated = prefs.getBoolean(getString(R.get("string", "pref_activated_key")) + key, true);
			if (createdByWizard && !activated) {
				//Check if account has been activated since
				activated = isAccountVerified(prefs.getString(getString(R.get("string", "pref_username_key")) + key, ""));
				if (activated) {
					SharedPreferences.Editor editor = prefs.edit();
					editor.putBoolean(getString(R.get("string", "pref_activated_key")) + key, true);
					editor.commit();
				} else {
					showDialog(CONFIRM_ID);
				}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		addPreferencesFromResource(R.get("xml", "preferences"));

		if (!getResources().getBoolean(R.get("bool", "hide_accounts"))) {
//			createDynamicAccountsPreferences();
			
			// Accounts have to be displayed to show add account button
			if (getResources().getBoolean(R.get("bool", "hide_add_account_button"))) {
				Preference addAccount = (Preference) getPreferenceScreen().getPreference(ADD_ACCOUNT_SETTINGS_ID);
				addAccount.setLayoutResource(R.get("layout", "hidden"));
			} else {
//				createAddAccountButton();
			...
			accounts.removeAll();
			accounts.setLayoutResource(R.get("layout", "hidden"));
		}
		
		if (getResources().getBoolean(R.get("bool", "hide_wizard"))) {
			Preference wizard = (Preference) getPreferenceScreen().getPreference(WIZARD_SETTINGS_ID);
			wizard.setLayoutResource(R.get("layout", "hidden"));
		} else {
...
//		addTransportChecboxesListener();
		...
		ecCalibratePref = (CheckBoxPreference) findPreference(R.get("string", "pref_echo_canceller_calibration_key"));
		...
		ecPref = (CheckBoxPreference) findPreference(R.get("string", "pref_echo_cancellation_key"));
		elPref = (CheckBoxPreference) findPreference(R.get("string", "pref_echo_limiter_key"));
		mencPref = (ListPreference) findPreference(R.get("string", "pref_media_encryption_key"));

		boolean fastCpu = Version.isArmv7();
		if (fastCpu) {
			detectAudioCodec(R.get("string", "pref_codec_ilbc_key"), "iLBC", 8000, false);
			findPreference(R.get("string", "pref_codec_speex16_key")).setEnabled(true);
			//findPreference(pref_codec_speex32_key)).setEnabled(enableIlbc);
		}
		findPreference(R.get("string", "pref_echo_limiter_key")).setEnabled(true);

//		initializeMediaEncryptionPreferences();
	
		detectAudioCodec(R.get("string", "pref_codec_amr_key"),"AMR", 8000, false);
        detectAudioCodec(R.get("string", "pref_codec_amrwb_key"),"AMR-WB", 16000, false);
		//detectAudioCodec(Common.getR("string", "pref_codec_silk8_key"),"SILK",8000, true);
		//detectAudioCodec(Common.getR("string", "pref_codec_silk12_key"),"SILK",12000, true);
		detectAudioCodec(R.get("string", "pref_codec_silk16_key"),"SILK",16000, true);
		detectAudioCodec(R.get("string", "pref_codec_silk24_key"),"SILK",24000, true);
		detectAudioCodec(R.get("string", "pref_codec_g729_key"),"G729",8000, true);
		...
		if (Hacks.hasBuiltInEchoCanceller()) {
			uncheckDisableAndHideCheckbox(R.get("string", "pref_echo_limiter_key"));
			uncheckDisableAndHideCheckbox(R.get("string", "pref_echo_cancellation_key"));
			uncheckDisableAndHideCheckbox(R.get("string", "pref_echo_canceller_calibration_key"));
		}
...
//			hidePreferenceCategory(R.get("string", "pref_tunnel_key"));
		}
		
		if (getResources().getBoolean(R.get("bool", "hide_camera_settings"))) {
...
//			enableVideo.setLayoutResource(R.get("layout", "hidden"));
	...
	private void hidePreferenceCategory(int key) {
		...
		p.removeAll();
		p.setLayoutResource(R.get("layout", "hidden"));
	}
	...
	private void initializeMediaEncryptionPreferences() {
		...
		if (!hasSrtp && !hasZrtp){
			...
			mencEntries.add(getString(R.get("string", "media_encryption_none")));
			mencEntryValues.add(getString(R.get("string", "pref_media_encryption_key_none")));
			if (hasSrtp){
				mencEntries.add(getString(R.get("string", "media_encryption_srtp")));
				mencEntryValues.add(getString(R.get("string", "pref_media_encryption_key_srtp")));
			}
			if (hasZrtp){
				mencEntries.add(getString(R.get("string", "media_encryption_zrtp")));
				mencEntryValues.add(getString(R.get("string", "pref_media_encryption_key_zrtp")));
			}
			...
			mencPref.setDefaultValue(getString(R.get("string", "media_encryption_none")));
			//mencPref.setValueIndex(mencPref.findIndexOfValue(getString(Common.getR("string", "media_encryption_none"))));
		}
	}
	...
	private void addTransportChecboxesListener() {
		...
		final List<CheckBoxPreference> checkboxes = Arrays.asList(
				findCheckbox(R.get("string", "pref_transport_udp_key"))
				,findCheckbox(R.get("string", "pref_transport_tcp_key"))
				,findCheckbox(R.get("string", "pref_transport_tls_key"))
				);
			...
			public boolean onPreferenceClick(Preference preference) {
				...
				if (((CheckBoxPreference) preference).isChecked()) {
					...
					/*Toast.makeText(LinphonePreferencesActivity.this,
							getString(Common.getR("string", "at_least_a_protocol")),
	...
	private synchronized void startEcCalibration() {
		try {
			...
			ecCalibratePref.setSummary(R.get("string", "ec_calibrating"));
			ecCalibratePref.getEditor().putBoolean(getString(R.get("string", "pref_echo_canceller_calibration_key")), false).commit();
		...
		mHandler.post(new Runnable() {
			public void run() {
				if (status == EcCalibratorStatus.Done) {
					ecCalibratePref.setSummary(String.format(getString(R.get("string", "ec_calibrated")), delayMs));
					ecCalibratePref.setChecked(true);

				} else if (status == EcCalibratorStatus.Failed) {
					ecCalibratePref.setSummary(R.get("string", "failed"));
	...
	private void manageCheckbox(int key, boolean value, boolean enabled, boolean hidden) {
		...
		if (hidden) box.setLayoutResource(R.get("layout", "hidden"));
	}
