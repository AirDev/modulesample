...
public abstract class AbstractCalleesActivity extends ListActivity implements LinphoneOnCallStateChangedListener, OnClickListener, LinphoneOnAudioChangedListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		View muteMic = findViewById(R.get("id", "toggleMuteMic"));
		muteMic.setOnClickListener(this);
		mMuteMicButton = (Checkable) muteMic;

		View speaker =  findViewById(R.get("id", "toggleSpeaker"));
...
//		}
	...
	protected void setCalleePicture(ImageView pictureView, LinphoneAddress address) {
		...
		//121104suzuki
		LinphoneUtils.setImagePictureFromUri(AbstractCalleesActivity.this, pictureView, null, R.get("drawable", "unknown_person"));
	}
	...
	protected abstract class CalleeListAdapter extends BaseAdapter {
		...
		protected final void registerCallDurationTimer(View v, LinphoneCall call) {
			...
			Chronometer timer = (Chronometer) v.findViewById(R.get("id", "callee_duration"));
			...
			callqualityHandler.postDelayed(new Runnable() {
				public void run() {
					...
					for (View v : viewsToUpdateCallQuality) {
						...
						ImageView speaking = (ImageView) v.findViewById(R.get("id", "callee_status_speeking"));
	...
	public void onClick(View v) {
		int id = v.getId();
		if (id == R.get("id", "toggleMuteMic")) {
			lc().muteMic(((Checkable) v).isChecked());
		}
		else if (id == R.get("id", "toggleSpeaker")) {
	...
	{
		ImageView qos = (ImageView) v.findViewById(R.get("id", "callee_status_qos"));
		if (!(qos.getVisibility() == View.VISIBLE)) {
			qos.setVisibility(View.VISIBLE);
		}
		if (quality >= 4) // Good Quality
		{
			qos.setImageDrawable(getResources().getDrawable(R.get("drawable", "stat_sys_signal_4")));
		}
		else if (quality >= 3) // Average quality
		{
			qos.setImageDrawable(getResources().getDrawable(R.get("drawable", "stat_sys_signal_3")));
		}
		else if (quality >= 2) // Low quality
		{
			qos.setImageDrawable(getResources().getDrawable(R.get("drawable", "stat_sys_signal_2")));
		}
		else if (quality >= 1) // Very low quality
		{
			qos.setImageDrawable(getResources().getDrawable(R.get("drawable", "stat_sys_signal_1")));
		}
		else // Worst quality
		{
			qos.setImageDrawable(getResources().getDrawable(R.get("drawable", "stat_sys_signal_0")));
		}
	}
}